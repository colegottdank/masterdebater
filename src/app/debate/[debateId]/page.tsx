...
export default function DebatePage() {
  ...
  const [isOwner, setIsOwner] = useState(true); // Default to true for new debates
  ...
  // Load debate from database or initialize new debate
  useEffect(() => {
    // Don't re-initialize if already done
    if (isInitialized) return;
    
    // Wait for user data to be loaded before initializing
    if (!user?.id) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.debate) {
            ...
            setIsOwner(data.isOwner !== undefined ? data.isOwner : true);
            setIsInitialized(true);
            setIsLoadingDebate(false);
            
            // If not owner, show a toast
            if (!data.isOwner) {
              console.log('Viewing read-only debate (not owner)');
            }
            return;
          }
  ...
  }, [debateId, router, searchParams, isInitialized, user?.id]);
                ...
                {isOwner ? (
                  ...
                  <div className="text-center py-4">
                    <p className="text-yellow-300 font-bold text-lg mb-3">
                      👀 You&apos;re viewing someone else&apos;s epic debate!
                    </p>
